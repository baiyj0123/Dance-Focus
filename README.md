...
上传成功后，系统会弹窗显示文件名、分辨率和时长，并自动把视频加载到预览区。

![上传成功与自动预览](docs/images/upload-success-preview.png)

## 最近更新

- 上传视频成功后增加浏览器弹窗提示，明确展示文件名、分辨率和视频时长
- 上传完成后自动加载首帧到预览区，不再需要用户点击 `播放` 才能看到画面
- 预览加载增加兼容兜底：如果浏览器没有主动解码首帧，会自动静音播放一瞬间再暂停以触发解码
- 视频上传失败、首帧解码失败、预览渲染失败时，都会给出弹窗和页面状态提示
- 修复部分浏览器点击 `导入素材` 没有反应的问题，改为按钮主动打开文件选择窗口
- 识别人物流程增加错误恢复，YOLO 服务未连接、模型未就绪或分析失败时不会卡在 `识别中`
- 页面增加轻量 toast 提示，成功、失败、预览异常等状态会在右上角同步反馈
...
2. 点击 `导入素材` 上传舞蹈视频
3. 等待上传成功弹窗出现，确认视频已经自动加载到预览区
4. 如需确认本地模型状态，可点击 `连接 YOLO`
5. 点击 `识别人物`，等待系统生成人物轨迹
6. 在轨迹列表中选择主舞者
7. 如画面中多人交叉或主角切换，可添加主角关键帧
8. 选择处理模式、遮挡样式、裁切留白、安全边距、导出帧率等参数
9. 点击导出，生成的视频会保存在 `rendered/` 目录，并在页面中提供下载入口
...
系统会先自动等待首帧解码；如果浏览器没有主动解码，会尝试静音播放一瞬间再暂停。如果仍然无法显示，会弹窗提示预览失败。

常见原因是视频编码不兼容，建议重新导出为：

- 容器：`MP4`
- 视频编码：`H.264`
- 音频编码：`AAC`

如果使用 `MOV` 或手机拍摄的 `HEVC/H.265` 视频，部分浏览器可能能读取文件信息，但无法正确绘制首帧。
...
- 文件是否完整、没有损坏
- 视频是否过大或网络磁盘读取过慢
- 浏览器是否为最新版 Chrome、Edge 或 Safari
- 视频是否为标准 `MP4(H.264 + AAC)`
